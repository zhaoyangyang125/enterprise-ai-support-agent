...
2026-09-16真实Provider：检查实际自动Schema，包含default/title/anyOf/additionalProperties；这些字段并非一概不受Google支持，因此没有把兼容猜测写成已确认根因。采用最小手写JSON Schema，保留本地严格校验及分状态脱敏错误日志。Google文档OCR采用延迟ADC，不手动读取凭据。首次回归发现单元测试误依赖可选SDK，改为模块注入以保持默认离线测试无需SDK；最终146 passed、2 skipped，前端4组通过。没有真实云验证。

2026-09-16：补充可复用的隔离图片演示启动器。真实浏览器已看到图片与Sheet Demo/B3来源；HTTP回归验证聊天、图片与跨用户拒绝。只读问题当前无法复现，不修改原库或系统权限。浏览器切换用户和真实模型效果仍需后续验收。

后续隔离验收：新增HTTP上传→真实Chroma→RAG引用→图片API集成测试。仅Vision结果为Fake，其余使用真实本地组件。验证跨用户拒绝及撤权后无证据/图片403。未重建原索引；原只读错误尚未定位到确切原因。
