...
### 7.16 「複雑なExcelをどのように解析しましたか」

以前の実装では、Sheetの最初の非空行をHeaderとして、後続行をすべて同じ表として扱っていました。しかし、実際の仕様書には、同じSheetの中に文書属性、複数の表、備考、章タイトルが存在します。

そこで、まず空白行と結合された章タイトルからRegionを分割し、各RegionをTitle、Key-Value、Table、Note、Paragraphに分類してから`ParsedBlock`へ変換しました。Tableでは複数行Headerを`CAN信号 / 信号名`のようなパスに結合し、Sheet、Section、Cell Rangeもmetadataとして保持します。

中文记忆：不是一上来切 Chunk，而是先找区域，再判断区域是什么结构，最后转换成统一 Block。

### 7.17 「結合セルをなぜ単純に全部展開しないのですか」

データ行の縦方向の結合は、機能IDなどの値を関連行へ継承する必要があります。一方、タイトルの横方向の結合をすべてのセルへコピーすると、同じタイトルが複数の項目として重複します。

そのため、原Workbookをunmergeせず、読み取り専用の論理Viewを作成しました。これにより、データ行では結合値を利用しながら、タイトルやRegion境界を保持できます。

中文记忆：数据合并需要继承，标题合并不能重复，所以不能无条件全部展开。

### 7.18 「Key-Valueと二列表をどう区別しましたか」

二列だけではKey-ValueかTableかを完全には判断できません。実装中にも、二行だけの`Rule / Value`表をKey-Valueとして誤判定するテスト失敗が発生しました。

そこで、曖昧な二列二行はTableとして扱い、二列Key-Valueは三行以上、多組のKey-Valueは空列による区切りを条件にしました。すべての形式を推測するのではなく、保守的なDefaultと回帰テストで動作を固定しています。

中文记忆：两列不等于 Key-Value；无法确定时使用保守默认值，并通过测试固定规则。

### 7.19 「テキストPDFをどのように解析しましたか」

まず、`pypdf`でページごとにテキストを抽出します。その後、各ページの上端と下端にある候補行だけを比較し、複数ページで繰り返されるヘッダーとフッターを除外します。

本文は見出しと段落に分け、`ParsedBlock`に`page`、`section`、`content_type`を保存します。Chunkはページをまたがないため、検索結果から元のページへ正確に戻ることができます。

中文记忆：先按页提取，再清理页边重复内容，最后识别标题和正文；不是把整份 PDF 直接切成固定长度。

### 7.20 「ヘッダーとフッターを誤って削除しないために何をしましたか」

本文全体から同じ文章を削除するのではなく、各ページの上端と下端の候補行だけを対象にしました。また、正規化後の文字列が全ページの60%以上、かつ2ページ以上に現れた場合だけ、繰り返し要素と判定します。

ページ番号はページごとに数字が変わるため、比較時だけ数字をプレースホルダーへ変換します。これにより、`Page 1 / 3`と`Page 2 / 3`を同じフッターパターンとして扱えます。

中文记忆：只删“页面边缘 + 多页重复”的文字，正文里偶然重复的话不会被全局删除。

### 7.21 「なぜPDFのChunkをページまたぎにしないのですか」

今回の優先目的は、回答の根拠をページ単位で正確に示すことです。二つのページを一つのChunkへ結合すると、一つのCitationでどちらのページを示すべきか曖昧になります。

そのため、同じSectionが続いていてもページごとにBlockを分けます。将来、検索品質の評価によって跨ページChunkが必要になった場合は、開始ページと終了ページを持つ別の設計として追加します。

中文记忆：当前优先保证来源定位准确；跨页不是永远不能做，而是要先设计 page range，不能悄悄合并。

### 7.22 「OCRや画像PDFにも対応していますか」

現在の実装対象はテキストレイヤーを持つPDFです。スキャンPDFのOCR、画像理解、複雑な表の視覚的な復元は今回の範囲に含めていません。

対応していない機能を実装済みとは説明せず、必要性、コスト、評価データを確認した後のRoadmapとして説明します。
...
### 7.26 「デモ画面をどのように実装しましたか」

既存システムがFastAPI中心のローカルアプリケーションであるため、HTML、CSS、JavaScriptをFastAPIから直接配信しました。小規模なデモ画面のために、別のReactプロジェクトやビルド環境は追加していません。

画面から、Mock認証情報、Chat、Metadata Filter、PDF/Excel Upload、処理状態、Citationを確認できます。UIは業務ロジックを持たず、既存APIを呼び出す役割に限定しています。

中文记忆：根据项目规模选择最简单合适的技术；界面只调用 API，不复制 Service 逻辑。

### 7.27 「文書Uploadの安全対策は何ですか」

現在は開発用のADMIN Roleを必須とし、PDFとXLSXだけを許可しています。空ファイルを拒否し、サイズ上限を10MBにしています。また、レスポンスにはサーバー側の保存Pathを含めません。

Upload成功時には、Uploadしたユーザーへread権限を追加します。その後も検索時にはBusiness Databaseから有効Versionと権限を確認するため、Chromaへ登録しただけでは無条件に検索できません。

中文记忆：上传权限、文件边界、返回信息、检索权限是四个不同的安全点。

### 7.28 「E2Eテストでどのような問題を発見しましたか」

最初の実Uploadでは、回答内容とPage番号は正しかったのですが、Citationのファイル名が一時ファイル名になっていました。API TestではFake Serviceを使っていたため、実際のStorageとDocumentService間の問題を検出できませんでした。

そこで、一時的な`source_path`と、ユーザーがUploadした`source_name`を別の引数として扱いました。修正後、Citationは`fictional_hmi_policy.pdf / Page 2`になり、同じ問題を防ぐ回帰テストも追加しました。

中文记忆：这是很好的面试案例——单元测试通过不等于完整链路正确，真实 E2E 找到了临时文件名污染 metadata 的问题。
